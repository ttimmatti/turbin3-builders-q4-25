...
describe("anchor-rock-paper-scissor", () => {
  ...
  const DEVNET_TEE_ENDPOINT = "https://tee.magicblock.app";
  ...
  let [player2ChoicePda] = anchor.web3.PublicKey.findProgramAddressSync(
    ...
    it("Airdrop SOL to Player 2", async () => {
        ...
        // if (providerEphemeralRollup.connection.rpcEndpoint.includes("tee")) {
            authTokenPlayer1 = await getAuthToken(DEVNET_TEE_ENDPOINT, player1);
            console.log("Player 1 Auth Token:", authTokenPlayer1);
            authTokenPlayer2 = await getAuthToken(DEVNET_TEE_ENDPOINT, player2);
            console.log("Player 2 Auth Token:", authTokenPlayer2);
            providerTeePlayer1 = new anchor.AnchorProvider(
              new anchor.web3.Connection(
                  DEVNET_TEE_ENDPOINT + "?token=" + authTokenPlayer1
              ),
              anchor.Wallet.local(),
            );
            providerTeePlayer2 = new anchor.AnchorProvider(
              new anchor.web3.Connection(
                DEVNET_TEE_ENDPOINT + "?token=" + authTokenPlayer2
              ),
              anchor.Wallet.local(),
            );
        // }
  ...
  it("Player 1 checks own choice", async () => {
    ...
    const accountInfo = await (providerEphemeralRollup).connection.getAccountInfo(player2ChoicePda);
  ...
  it("Sneak Player 1 Choice"  , async () => {
    ...
    const accountInfo = await (providerEphemeralRollup).connection.getAccountInfo(player1ChoicePda);
    if (accountInfo === null) {
