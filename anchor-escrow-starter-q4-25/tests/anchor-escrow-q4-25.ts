...
describe("anchor_escrow_q4_25", () => {
  ...
  before(async () => {
    ...
    mintB = await createMint(provider.connection, maker, taker.publicKey, null, 0);
    ...
    await provider.sendAndConfirm(makerAtaATx, [maker]);
    await mintTo(provider.connection, maker, mintA, makerAtaA, maker.publicKey, depositAmount * 2);
